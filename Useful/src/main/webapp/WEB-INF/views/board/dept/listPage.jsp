...
<%@include file="../../login/Sidebar.jsp" %>
		...
		<div id="page-wrapper" style="height: 100%;">
			<div class="row" style="height: 100%;">
				<div class="col-lg-12">
					<h1 class="page-header">부서별게시판</h1>
				</div>
				<!-- /.col-lg-12 -->
			</div>
								...
								<tbody>
									<c:forEach items="${list }" var="BoardVO">
										<tr>
											...
											<td><a href="/useful/board/dept/readPage${pageMaker.query(pageMaker.cri.page) }&serial=${BoardVO.serial}">${BoardVO.title }</a></td>
									...
									<!-- 페이지번호 -->
										<div>
											...
											<span><a href="/useful/board/dept/listPage${pageMaker.serach(pageMaker.startPage -1) }">이전</a></span>
											
											</c:if>
												
												<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
												<span <c:out value="${pageMaker.cri.page==idx?'class=active':'' }"/>>
												<a href="/useful/board/dept/listPage${pageMaker.query(idx) }">${idx }</a>
												</span>
												</c:forEach>
												
													<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
											<span><a href="/useful/board/dept/listPage${pageMaker.serach(pageMaker.endPage +1)}">다음</a></span>
											</c:if>
											</div>
											</p>
										</div>
...
$(document).ready(function(){
	$("#register").on("click",function(event){
		...
		self.location="/useful/board/dept/createPage";
	});
