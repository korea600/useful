...
<script type="text/javascript">
	...
	function search(){
		...
		location.href='${pageContext.request.contextPath}/approval/liststatus${pagemaker.makeQuery(1)}&searchBy='+searchBy+'&keyword='+keyword;
	}
</script>
</head>
<body>
		...
		<div class='col-lg-12'>
			<div class='panel panel-default'>
				<div class='panel-heading'>
					<select name='searchBy' onchange="selectSearchType()" class='form-control-static'>
						<option value=''>선택</option>
						<option value='status'>결재진행상태 </option>
						<option value='writer_name'>작성자</option>
					</select>
					<select name='selectStatus' style="display: none;" class='form-control-static'>
					</select>
					<input type='text' name='keyword' style="display: none;"  class='form-control-static'>
					<input type='button' id='search' value='검색' onclick='search()' class='btn btn-warning'>
				</div>
		...
		<div class='dataTables_paginate paging_simple_numbers' id='dataTables-example_paginate' style="text-align: center;">
			<ul class='pagination'>
				<c:if test="${pagemaker.prev}">
					<li class='paginate_button' id='dataTables-example_previous'><a href="${pageContext.request.contextPath}/approval/liststatus${pagemaker.makeSearch(pagemaker.startPage-1)}">이전</a></li>
				</c:if>
				<c:forEach begin='${pagemaker.startPage}' end="${pagemaker.endPage }" var='i'>
					<c:if test="${pagemaker.cri.page==i}"><li class='paginate_button active'><a href='#'>${i}</a></li></c:if>
					<c:if test="${pagemaker.cri.page!=i}"><li class='paginate_button'><a href="${pageContext.request.contextPath}/approval/liststatus${pagemaker.makeSearch(i)}"> ${i} </a></li></c:if>
				</c:forEach>
				<c:if test="${pagemaker.next && pagemaker.endPage>0 }">
					<li class='paginate_button' id='dataTables-example_next'><a href="${pageContext.request.contextPath}/approval/liststatus${pagemaker.makeSearch(pagemaker.endPage+1)}">다음</a></li>
				</c:if>				
			</ul>
		</div>
	</div>
